...
**TEST PIPELINE 2 TRÊN JETSON AGX ORIN. TĂNG SOFT LOCK THRESHOLD.**
---
...
| `soft_lock_threshold`| **0.60** | Ngưỡng điểm Lọc Thô (Cosine) dùng GASNet. | ⬇️ Giảm = Khóa lầm nhiều rác. ⬆️ Tăng = Bỏ lọt mục tiêu thật khi mờ/nhiễu. |
...
| Tiêu chí Đánh giá (Metrics) | Kết quả hiện tại (Ngày 2026-08-25) | Kết quả mong muốn |
| :--- | :--- | :--- |
| **Avg CNN Feature Extraction** (ms) | `43.10` | Càng thấp càng tốt (< 15ms) |
| **Avg System Throughput** (FPS) | `23.25` | > 30 FPS là đạt chuẩn Real-time |
| **Avg Mamba + Head Time** (ms) | `9.39` | Càng thấp càng tốt (< 10ms) |
| **Re-acquisition Latency** (frames) | `23.37` | Càng thấp càng tốt (Khóa lại nhanh) |
| **False Alarms (Fine Fails)** | `25830` | Càng thấp càng tốt |
...
| :--- | :--- | :--- |
| **Total Genuine Queries** | `[Điền số]` | Số lần test con thật. |
| **Total Imposter Queries** | `[Điền số]` | Số lượng con giả bị đem ra test. |
| **Avg Genuine Similarity** | `[Điền số]` | Phải gần 1.0 (vd: 0.95+). Nếu thấp -> Model chưa bắt được đặc trưng thật. |
| **Avg Imposter Similarity** | `[Điền số]` | Phải cực kỳ thấp (vd: < 0.6). Nếu cao -> Model nhận diện kém, cần train lại hoặc nâng `reid_threshold`. |
| **Average Margin (Gen - Imp)** | `[Điền số]` | Khoảng cách giữa thật và giả. Càng cao (vd: 0.4+) thì hệ thống càng phân biệt tốt. |
| **False Rejection Rate (FRR)** | `[Điền số] %` | Tỷ lệ nhận diện trượt con thật. Nếu cao -> Phải hạ `reid_threshold` xuống. |
| **False Acceptance Rate (FAR)**| `[Điền số] %` | Tỷ lệ nhận nhầm con giả. Nếu cao -> Phải nâng `reid_threshold` lên hoặc thu hẹp `num_frames`. |
