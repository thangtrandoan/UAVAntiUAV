# Báo Cáo Cấu Hình & Kết Quả Đánh Giá Pipeline ReID (Pipeline 2)
...
- **`T2_SEARCH`**: Phát hiện các vật thể lạ xuất hiện. 
  - Bước 1: **Lọc Thô (Coarse Filter)** - Chấm điểm bề ngoài (GASNet) của từng vật thể (>= `soft_lock_threshold`).
  **Dùng vector 3072-dim cho Coarse** | Khi Coarse, nhân bản 1 ảnh ra k frame → chạy Mamba → lấy vector 3072-dim đã qua BNNeck của Head | Tận dụng BNNeck đã train riêng cho UAV, điểm phân biệt tốt nhất | Tốn thêm ~10ms Mamba cho mỗi lần Coarse |
...
*Ghi chú: Thay đổi các tham số này trong `configs/config_jetson.yaml` sẽ làm thay đổi trực tiếp độ trễ, tốc độ chạy và tỷ lệ nhận nhầm của hệ thống.*
...
| `soft_lock_threshold`| **0.50** | Ngưỡng điểm Lọc Thô (Cosine) dùng GASNet. | ⬇️ Giảm = Khóa lầm nhiều rác. ⬆️ Tăng = Bỏ lọt mục tiêu thật khi mờ/nhiễu. |
...
*(Dữ liệu được trích xuất từ `infer_output/infer_pipeline_2/summary_metrics.txt`)*
...
**Lệnh chạy:** `python3 infer.py --config configs/config_jetson.yaml`

| Tiêu chí Đánh giá (Metrics) | Kết quả hiện tại (Pipeline 2) | Kết quả mong muốn |
| :--- | :--- | :--- |
| **Avg CNN Feature Extraction** (ms) | `22.71 ms` | Càng thấp càng tốt (< 15ms) |
| **Avg System Throughput** (FPS) | `45.64 FPS` | > 30 FPS là đạt chuẩn Real-time |
| **Avg Mamba + Head Time** (ms) | `4.27 ms` | Càng thấp càng tốt (< 10ms) |
| **Re-acquisition Latency** (frames) | `13.33 frames` | Càng thấp càng tốt (Khóa lại nhanh) |
| **False Alarms (Fine Fails)** | `13967` | Càng thấp càng tốt |
...
**Lệnh chạy:** `python3 evaluate_reid_robustness.py --config configs/config_jetson.yaml`

| Tiêu chí Đánh giá (Metrics) | Kết quả hiện tại | Phân tích & Hướng chỉnh sửa |
| :--- | :--- | :--- |
| **Total Genuine Queries** | `[Chưa chạy test]` | Số lần test con thật. |
| **Total Imposter Queries** | `[Chưa chạy test]` | Số lượng con giả bị đem ra test. |
| **Avg Genuine Similarity** | `[Chưa chạy test]` | Phải gần 1.0 (vd: 0.95+). Nếu thấp -> Model chưa bắt được đặc trưng thật. |
| **Avg Imposter Similarity** | `[Chưa chạy test]` | Phải cực kỳ thấp (vd: < 0.6). Nếu cao -> Model nhận diện kém, cần train lại hoặc nâng `reid_threshold`. |
| **Average Margin (Gen - Imp)** | `[Chưa chạy test]` | Khoảng cách giữa thật và giả. Càng cao (vd: 0.4+) thì hệ thống càng phân biệt tốt. |
| **False Rejection Rate (FRR)** | `[Chưa chạy test] %` | Tỷ lệ nhận diện trượt con thật. Nếu cao -> Phải hạ `reid_threshold` xuống. |
| **False Acceptance Rate (FAR)**| `[Chưa chạy test] %` | Tỷ lệ nhận nhầm con giả. Nếu cao -> Phải nâng `reid_threshold` lên hoặc thu hẹp `num_frames`. |
